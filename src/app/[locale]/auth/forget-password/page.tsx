...
const ForgetPass = () => {
  ...
  const handleOTPComplete = async (otp: string) => {
    ...
    try {
      const response = await fetch("http://localhost/quttouf-backend/api/user/verify-otp", {
        method: "POST",
        body: formData,
      });
    ...
    <>
      <div className='space-y-8 mb-8'>
        ...
        <div className='space-y-2'>
          ...
          <p className='text-[#656C77] text-[16px] font-[400]'>{t("otp_message")}</p>
          <p className='text-[#40444C] text-[16px] font-[400]'>gmail@example.com</p>
        </div>
      </div>
      <div>
        {!showOTP ? (
          <SendOTP setShowOTP={setShowOTP} setCountry_code={setCountry_code} setPhone={setPhone} />
        ) : (
          <div className='flex flex-col items-center'>
            ...
            <p className='flex items-center text-center text-[#656C77] text-[16px] font-[400] mb-4'>
              {t("resend_code")} <Button variant='link' className='text-[#009444] text-[16px]'>{t("click_resend")}</Button>
            </p>
