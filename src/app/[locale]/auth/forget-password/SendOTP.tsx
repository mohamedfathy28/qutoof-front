...
const SendOTP = ({ setShowOTP, setCountry_code, setPhone }: ISendOTPprops) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost/quttouf-backend/api/user/countries");
  ...
  const handleSendOTP = async (values: { country_code: string; phone: string }) => {
    ...
    try {
      const response = await fetch("http://localhost/quttouf-backend/api/user/resend-otp", {
        method: "POST",
        body: formData,
      });
