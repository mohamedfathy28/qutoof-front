...
const RenderAwaitingApproval = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`http://localhost/quttouf-backend/api/user/awaiting-approval?per_page=${PerPage}&page=${CurrentPage}`, {
          headers: myHeaders,
        });
...
<div className='flex flex-col lg:grid grid-cols-1 lg:grid-cols-3 gap-6'>
      ...
      <div className='col-span-3'>
        {data?.length !== 0 ? <Pagination currentPage={CurrentPage} totalPages={totalPages || 1} onPageChange={(t) => setCurrentPage(t)} /> : ""}
      </div>
    </div>
    </>
  )
