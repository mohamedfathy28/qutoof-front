...
const RenderCurrentlyOwned = () => {
    ...
    const handleSell = async (sectorid: number | undefined) => {
        ...
        try {
            const response = await fetch("http://localhost/quttouf-backend/api/user/sectors/sell-shares", {
                method: "POST",
                headers: myHeaders,
                body: formData,
            });
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch(`http://localhost/quttouf-backend/api/user/sectors/currently-owned?per_page=${PerPage}&page=${CurrentPage}`, {
                headers: myHeaders,
            });
            const result = await response.json();
            setData(result.data);
