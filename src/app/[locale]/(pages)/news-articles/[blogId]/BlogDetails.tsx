...
const BlogDetails = ({ blogId }: IProps) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				const response = await fetch(
					`https://www.quttouf.com/api/user/blogs/${blogId}`,
					{
						headers: myHeaders,
					}
				);
		...
		const fetchBlogs = async () => {
			...
			myHeaders.append("Accept-Language", direction == "ltr" ? "en" : "ar");
			try {
				const response = await fetch("https://www.quttouf.com/api/user/blogs", {
					headers: myHeaders,
				});
				const result = await response.json();
			...
			<div className='mx-auto max-w-[90%] sm:max-w-xl md:max-w-2xl lg:max-w-4xl xl:max-w-6xl 2xl:max-w-7xl my-20 md:my-32 space-y-20 lg:space-y-32'>
				<div className='grid grid-cols-3 gap-8 lg:gap-16'>
					<div className='col-span-3 lg:col-span-3'>
						...
						{/*  /!*</div>*!/*/}
						{/*</div>*/}
						<h2 className='text-[30px] text-[#1F1E17] font-[700] my-4'>
							{data?.title}
						</h2>
						<p className='text-[16px] font-[500] leading-[30px] text-[#656565] mb-6'>
							{data?.content}
						</p>
					</div>
						...
						<div className='bg-[#EAF8F0] rounded-[10px] p-8'>
							...
							<ul className='space-y-6'>
								<li>
									<Link
										href={""}
										className='text-[16px] text-[#42423F] font-[500]'
									>
										Agriculture
									</Link>
								</li>
				...
				<div>
					...
					<div className='grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8'>
						{Blogs &&
							Blogs.slice(0, 3).map((blogInfo: IBlog) => (
