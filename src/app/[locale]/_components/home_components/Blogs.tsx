			...
			<div className='flex flex-col lg:flex-row items-stretch gap-8'>
				...
				>
					...
					<div className='flex flex-col justify-end gap-4 md:gap-6 text-white absolute top-0 left-0 w-full h-full bg-gradient-to-b from-black/0 to-black/70 rounded-[8px] p-6 md:p-16'>
						...
						<Link
							href={`/news-articles/${data ? data[0].id : 1}`}
							className='flex items-start w-fit gap-2 hover:gap-4 transition-all duration-300 leading-[14px] text-[14px]'
						>
							{t("ContinueReading")}{" "}
							<svg
				...
				{data &&
					data.slice(1, 3).map((blogInfo: IBlog) => (
						<div
							key={blogInfo.id}
							data-aos='fade-left'
							data-aos-duration='500'
